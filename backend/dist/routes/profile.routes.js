...
const aiPreferenceModel = prismaRuntime.userAiPreference;
const vocabularyNotebookModel = prismaRuntime.vocabularyNotebook;
const vocabularyItemModel = prismaRuntime.vocabularyNotebookItem;
const speakingEvaluationModel = prismaRuntime.speakingEvaluation;
const speakingQuestionModel = prismaRuntime.speakingQuestionBank;
const fallbackAiPreferenceStore = new Map();
const fallbackVocabularyStore = new Map();
function createFallbackId(prefix) {
    return `${prefix}-${Date.now()}-${Math.round(Math.random() * 100000)}`;
}
function isAiPreferenceModelReady() {
    return (typeof aiPreferenceModel?.findUnique === 'function' &&
        typeof aiPreferenceModel?.upsert === 'function');
}
function isVocabModelReady() {
    return (typeof vocabularyNotebookModel?.findMany === 'function' &&
...
function getFallbackNotebooks(userId) {
    return fallbackVocabularyStore.get(userId) ?? [];
}
const fallbackSpeakingQuestionBank = [
    {
        id: 'spk-p1-1',
        part: 1,
...
function getPastSevenDays(now) {
    return Array.from({ length: 7 }, (_, index) => addUtcDays(startOfUtcDay(now), -6 + index));
}
function clamp(value, min, max) {
    return Math.min(max, Math.max(min, value));
}
function parsePreferredName(fullName) {
    if (!fullName)
        return null;
    return fullName.trim().split(/\s+/)[0] ?? null;
}
function buildSpeakingEvaluation(transcript, pronunciationSignal) {
    ...
    const grammarIssueCount = grammarPenaltyPatterns.reduce((total, pattern) => total + (cleaned.match(pattern)?.length ?? 0), 0);
    ...
    const pronunciationBand = clamp(pronunciationSignal !== undefined ? 4 + (pronunciationSignal / 100) * 5 : fluencyBand - 0.15, 4, 9);
    const overallBand = clamp(fluencyBand * 0.3 + grammarBand * 0.25 + pronunciationBand * 0.25 + lexicalBand * 0.2, 4, 9);
    const metricOrder = [
    ...
    const strengths = [];
    if (lexicalBand >= 6.5)
        strengths.push('Good lexical range with topic-relevant wording.');
    if (fluencyBand >= 6.5)
        strengths.push('Stable speaking rhythm and pacing.');
    if (grammarBand >= 6.5)
        strengths.push('Generally controlled grammar across responses.');
    if (pronunciationBand >= 6.5)
        strengths.push('Pronunciation is understandable and mostly clear.');
    if (strengths.length === 0)
        strengths.push('You kept speaking and maintained communication throughout.');
    const weaknesses = [];
    if (fillerRatio > 0.05)
        weaknesses.push('Frequent filler usage reduced fluency precision.');
    if (grammarIssueCount > 0)
        weaknesses.push('Repeated grammar slips affected sentence accuracy.');
    if (avgSentenceLength < 7)
        weaknesses.push('Ideas were often short and underdeveloped.');
    if (weaknesses.length === 0)
        weaknesses.push('Main focus is polishing detail-level accuracy for a higher band.');
    return {
...
router.delete('/attempts/:attemptId', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
...
router.post('/ai-report', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
...
router.post('/ai-chat', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
...
router.get('/ai-preferences', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
    const user = await prisma.user.findUnique({
        where: { id: userId },
        select: {
    ...
    if (!isAiPreferenceModelReady()) {
        ...
        return res.json({
            ...
            lastAssistantLanguage: fallbackPreference?.lastAssistantLanguage ?? fallbackPreference?.preferredLocale ?? 'en',
...
router.put('/ai-preferences', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
...
router.post('/vocab-notebook', requireAuth, asyncHandler(async (req, res) => {
    ...
    const preference = isAiPreferenceModelReady()
...
router.patch('/vocab-notebook/:notebookId', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
...
router.delete('/vocab-notebook/:notebookId', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
    if (!isVocabModelReady()) {
        const current = getFallbackNotebooks(userId);
...
router.post('/vocab-notebook/:notebookId/items', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
...
router.patch('/vocab-notebook/items/:itemId', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
...
router.delete('/vocab-notebook/items/:itemId', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
    if (!isVocabModelReady()) {
        const current = getFallbackNotebooks(userId);
        let found = false;
...
router.post('/ai-realtime/session', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
    ...
    const instructions = payload.mode === 'mock'
...
router.get('/speaking/questions', requireAuth, asyncHandler(async (req, res) => {
...
router.post('/speaking-evaluate', requireAuth, asyncHandler(async (req, res) => {
    const userId = req.user.id;
