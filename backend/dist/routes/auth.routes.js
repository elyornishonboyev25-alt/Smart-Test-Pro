...
import { generateTokenId, getTokenExpiryDate, hashToken, signAccessToken, signRefreshToken, verifyRefreshToken, } from '../utils/jwt.js';
...
router.post('/google', authRateLimit, validateBody(googleAuthSchema), asyncHandler(async (req, res) => {
...
router.get('/google/config', authRateLimit, asyncHandler(async (_req, res) => {
    return res.json({
        enabled: Boolean(resolvedGoogleClientId),
        clientId: resolvedGoogleClientId || null,
    });
}));
router.post('/refresh', authRateLimit, validateBody(refreshSchema), asyncHandler(async (req, res) => {
