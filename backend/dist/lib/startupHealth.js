...
export async function runStartupHealthChecks() {
    ...
    for (const modelName of REQUIRED_AI_MODELS) {
        const delegate = prismaRuntime[modelName];
    ...
    try {
        const rows = await prisma.$queryRawUnsafe('SELECT migration_name FROM "_prisma_migrations" WHERE migration_name = $1', REQUIRED_AI_MIGRATION);
