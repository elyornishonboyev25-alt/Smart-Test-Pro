...
function toSafeSyncSlug(externalTestId, track) {
...
router.post('/reading-sync', requireAuth, validateBody(readingSyncSchema), asyncHandler(async (req, res) => {
    ...
    const attemptResponse = await prisma.$transaction(async (tx) => {
        ...
        if (!existingActivity) {
            ...
            const nextAverageScore = (existingActivity.averageScore * existingActivity.testsCompleted + finalScore) / nextTests;
...
router.post('/:id/submit', requireAuth, validateBody(submitAttemptSchema), asyncHandler(async (req, res) => {
    const userId = req.user.id;
    ...
    const attemptResponse = await prisma.$transaction(async (tx) => {
        ...
        if (!existingActivity) {
            ...
            const nextTests = existingActivity.testsCompleted + 1;
            const nextAverageScore = (existingActivity.averageScore * existingActivity.testsCompleted + score.finalScore) / nextTests;
            await tx.dailyActivity.update({
                where: {
                    userId_activityDate: {
                        userId,
...
router.post('/', requireAuth, requireRole(['ADMIN']), validateBody(createTestSchema), asyncHandler(async (req, res) => {
    const payload = req.body;
