...
export function isAiCoachProviderError(error) {
...
function buildFallbackReport(context) {
    ...
    return {
        ...
        model: env.HF_MODEL,
        source: 'fallback',
        executiveSummary: `Your profile is stable with a current skill power of ${context.overall.skillPower.toFixed(1)} and recent average score around ${recentMean}. ` +
        ...
        momentumAssessment: `Cumulative XP momentum is positive, while score consistency needs tighter calibration. ` +
...
async function requestHfReport(systemPrompt, userPrompt) {
...
export async function precomputeAiCoachReport(userId) {
    try {
